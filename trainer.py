...
from layers import *
from core.R_MSFM import R_MSFM6, R_MSFM3
import datasets
import networks
from datasets.nuScenes_dataset import NuScenesDataset
import sys
sys.path.append('./SCI')
from SCI.model import Network as SCINetwork


torch.cuda.set_device(1)  # 强制使用cuda
cuda_device = "cuda:1"
...
class Trainer:
    def __init__(self, options):
        ...
        self.device = torch.device("cpu" if self.opt.no_cuda else cuda_device)
        ...
        if self.opt.iters == 6:
            if self.opt.x:
                ...
                print("use R-MSFM6")
            self.models["depth"] = R_MSFM6(self.opt.x,
            ...
            if self.opt.x:
                ...
                print("use R-MSFM3")
            self.models["depth"] = R_MSFM3(self.opt.x,
                                           use_moga=self.opt.use_moga,
                                           use_cga=self.opt.use_cga,
                                           use_wtdown=self.opt.use_wtdown,
                                           use_bra=self.opt.use_bra)
        ...
        self.scheduler = optim.lr_scheduler.OneCycleLR(self.optimizer, self.opt.learning_rate, epochs=40, div_factor=25,
                                                       steps_per_epoch=len(train_dataset) // self.opt.batch_size,
        ...
        # 使用预训练权重
        if self.opt.pre_weight:
            print("use pre weight: ", self.opt.pre_weight)
            self.load_model()
    ...
    def load_model(self):
        ...
        self.opt.pre_weight = os.path.expanduser(self.opt.pre_weight)

        assert os.path.isdir(self.opt.pre_weight), \
            "Cannot find folder {}".format(self.opt.pre_weight)
        print("loading model from folder {}".format(self.opt.pre_weight))

        for n in self.opt.models_to_load:
            print("Loading {} weights...".format(n))
            path = os.path.join(self.opt.pre_weight, "{}.pth".format(n))
            model_dict = self.models[n].state_dict()
            pretrained_dict = torch.load(path, map_location='cpu')
            pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
            model_dict.update(pretrained_dict)
